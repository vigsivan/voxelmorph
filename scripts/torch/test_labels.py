...
save_dir2 = Path("/home/vsivan/warped_with_torch_aug_seg/")
...
for img_pair, seg_pair in zip(img_pairs, seg_pairs):
    ...
    warped_seg, deformation_field = model(segs[0], segs[1])
    dice = vxm.py.utils.dice(warped_seg.detach().cpu().numpy(), segs[1].detach().cpu().numpy(), labels=labels)
    warped_image = model.transformer(images[0], deformation_field)
