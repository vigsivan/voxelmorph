...
save_dir = Path("/home/vsivan/warped_with_torch_pairwise/")
save_dir2 = Path("/home/vsivan/warped_with_torch_pairwise_seg/")
save_dir3 = Path("/home/vsivan/warped_with_torch_pairwise_fseg/")
...
for img_pair, seg_pair in zip(img_pairs, seg_pairs):
    ...
    images = [tio.ScalarImage(i).data.unsqueeze(-1).to(device).float().permute(0, 4, 1, 2, 3)
            for i in img_pair]
    segs = [tio.ScalarImage(i).data.unsqueeze(-1).to(device).float().permute(0, 4, 1, 2, 3)
            for i in seg_pair]

    warped_image, deformation_field, warped_seg = model(images[0], images[1], segs[0])
    chans = deformation_field.shape[1]
    deformation_field = deformation_field.detach()
    model = model.eval()
    with torch.no_grad():
        for c in range(chans):
            df = torch.clone(deformation_field)
            df[:,c,...] = 0
            warped = model.vxm.transformer(images[0], df)
            wtio = tio.ScalarImage(tensor=einops.rearrange(warped, 'b f d w h -> (b f) d w h').detach().cpu())
            wtio.save(save_dir/(f"c_{c}" + Path(img_pair[0]).name))
