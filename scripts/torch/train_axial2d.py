...
def semisupervised2d(vol_names, seg_names, labels, atlas_file=None, downsize=2):
    """
    ...
    gen = vxm.generators.volgen(vol_names, segs=seg_names, np_var='vol')
    zeros = None
    ...
    if atlas_file:
        ...
        trg_seg = vxm.py.utils.load_volfile("/home/vsivan/atlas_com/PAM50_t1_crop_seg.nii.gz",np_var='seg',
                                     add_batch_axis=True, add_feat_axis=True)
    ...
    while True:
        ...
        # indices_with_seg_src = np.where(np.max(src_seg, axis=(0,1,3,4)))[0].ravel()
        indices_with_seg_src = np.where(np.max(src_seg, axis=(0,1,2,4)))[0].ravel()
        
        sli_src = indices_with_seg_src[len(indices_with_seg_src)//2]

        src_vol, src_seg = src_vol[...,sli_src,:], src_seg[...,sli_src,:]
        src_seg = einops.repeat(src_seg, 'b w h f -> b w h d f', d=1)
        ...
        if not atlas_file:
            ...
            indices_with_seg_trg = np.where(np.max(trg_seg, axis=(0,1,2,4)))[0].ravel()
            sli_trg = indices_with_seg_trg[len(indices_with_seg_trg)//2]
            trg_vol, trg_seg = trg_vol[...,sli_trg,:], trg_seg[...,sli_trg,:]
            trg_seg = einops.repeat(src_seg, 'b w h f -> b w h d f', d=1)
            trg_seg = split_seg(trg_seg)
            trg_seg = trg_seg[...,0,:]
...
generator = semisupervised2d(train_imgs, train_segs, labels=train_labels, atlas_file=args.atlas)
